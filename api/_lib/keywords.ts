import { isStopword } from "./rules.js";

/** Copy of src/ats/keywords.ts — the serverless build cannot import from src/.
 *  Kept identical on purpose: the agent's exit condition has to be measured by
 *  the same matcher the app will run afterwards, or CVisor hands back a CV that
 *  CVsible's own scan then criticises. ./keywords.test.ts fails if they drift.
 *
