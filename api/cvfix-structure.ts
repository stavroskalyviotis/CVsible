...
import {
  CVFIX_DAILY_LIMIT,
  CVFIX_MAX_TOKENS,
  CVFIX_STRUCTURE_MODEL,
  LANGUAGE_LEVELS,
  MAX_RESUME_TEXT_CHARS,
  MAX_ROUNDS_PER_CV,
} from "./_lib/constants.js";

/** Phase one of fixing an uploaded CV: get the candidate's own words out of a
 *  mangled two-column extraction and into the right fields, changing nothing.
 *
 *  This is deliberately separate from ./cvfix.ts, which rewrites. Structuring
 *  is reading comprehension and must not touch a word; rewriting is
 *  composition and is reviewed change by change. Running them as one step is
 *  what made it impossible to tell whether CVfix had understood the CV or
 *  merely reworded it. A CV built in CVsible is already structured and skips
 *  this entirely. */
